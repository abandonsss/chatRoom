...
void* MyReactor::main_loop(void *p)
{
    ...
    my_logger->info("server start run.....");
...
void* MyReactor::accept_thread_proc(void* args)
{
    ...
    while(!pReactor->m_bStop)
    {
        ...
        struct epoll_event e;
        memset(&e, 0, sizeof(e));
        e.events = EPOLLIN | EPOLLRDHUP | EPOLLET;//EPOLLET epoll以边缘触发模式工作，配合O_NONBLOCK使用，高效 水平模式
        e.data.fd = newfd;
        /* 添加进epoll的兴趣列表 */
            ...
            else if(buff[0] == LOGIN)
            {
                ...
                else
                {
                      ...
                      if(strcmp(pReactor->sqlrow[0], password) == 0)
                      {
						...
						pReactor->sqlQuery(query);
                        my_logger->info("user:{} have client to LOGIN",name);
                      }
                      else
                      {
...
int MyReactor::connect()
{
    ...
    mysql = mysql_real_connect(mysql, IP, "root", "123456", "ChatRoom", 0, NULL, 0);//连接到数据库
